...
import { durationLabel, escapeHtml, formatTime, parseDate } from "./flight-tracker-card-formatters.js";

export function renderTimelineCard(title, state) {
  if (!state || !state.attributes) {
    return `${styles()}${timelineEmptyCard()}`;
  }

  const attrs = state.attributes || {};
  const segments = Array.isArray(attrs.segments) ? attrs.segments : [];
  if (!segments.length) {
    return `${styles()}${timelineEmptyCard(attrs)}`;
  }
...
function timelineStatus(segment) {
  ...
  return timelineStatusHtml(status);
}
